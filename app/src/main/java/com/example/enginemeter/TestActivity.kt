...
import com.example.enginemeter.databinding.ActivityTestBinding
...
class TestActivity : AppCompatActivity() {
    ...
    private fun btnSave(){
        binding.btnCalculate.setOnClickListener {
            if (binding.etKecepatan.text.toString().isEmpty()){
                Toast.makeText(applicationContext, "Kecepatan tidak boleh kosong...", Toast.LENGTH_SHORT).show()
            } else if (binding.etDurasi.text.toString().isEmpty()){
                Toast.makeText(applicationContext, "Durasi tidak boleh kosong...", Toast.LENGTH_SHORT).show()
            } else {
                speed = binding.etKecepatan.text.toString().toDouble()
                time = binding.etDurasi.text.toString().toDouble()
                ...
                try {
                    CoroutineScope(Dispatchers.IO).launch {
                        db.entityDao().createData(MainModel(0, speed, time, final_result))
                    }
                    showAnswer(final_result, string_result)
                }catch (e: Exception){
    ...
    private fun showAnswer(s: Double, t: String){
        val alert = AlertDialog.Builder(this)
        alert.apply {
            setTitle("Calculated w/ Fuzzy Logic")
            setMessage("""
                Suhu : $s celsius
                
                $t
                
                fuzzy kecepatan dan durasi
                ${rSpeedDouble.toList()} => ${rSpeedString.toList()}
                ${rTimeDouble.toList()} => ${rTimeString.toList()}
                
                inference
                ${inferenceDouble.toList()} => ${inferenceString.toList()}
                
                disjungtion
                ${disjunctionDouble.toList()} => ${disjunctionString.toList()}
            """.trimIndent())
            setPositiveButton("Mengerti"){dialog, which ->
                dialog.dismiss()
                startActivity(Intent(this@TestActivity, HistoryActivity::class.java).also { finish() })
            }
            show()
        }
    }
